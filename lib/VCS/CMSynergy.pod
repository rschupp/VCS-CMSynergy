...
representing the task associated with th object. The array may be
empty if no associated tasks exist.
...
If this option is set, the mapping will also include
members of subprojects. It is "off" by default.
...
of the option of the same name for L</traverse_project>.
...
=head2 folder_object, task_object

  $folder_obj = $ccm->folder_object("123");
  $task_obj = $ccm->task_object("42#mydb");

Create a C<VCS::CMSynergy::Object> (with cvtype "folder" or "task", resp)
from the I<displayname> of a folder or task, resp. (which is typically a number
or a number followed by the DCM database_id).
...
C<object_from_cvid> uses

  $ccm->property(objectname => "\@=$cvid")

to determine the objectname corresponding to C<$cvid>.

=head2 base_admin, base_model, dcm_admin, cvtype, attype
...
=item C<base_admin>, C<base_model>, C<dcm_admin>
