...
- better name for mkInstallation, document it
  OR replace this CLASS with a server/installation OBJECT

- IPC::Open3 croaks on NT if stdout or stderr is closed on entry
  reason is probably that perl opens the script file read-only and it gets
